...
async def _run_publish_teams() -> None:
    ...
    if not has_graph and not has_webhook:
        print(
            "Teams delivery not configured. Set TEAMS_WEBHOOK_URL or TEAMS_TEAM_ID + TEAMS_CHANNEL_ID."
        )
        return
...
async def _run_audit() -> None:
    ...
    async with async_session_factory() as db:
        ...
        rows = (
            await db.execute(
                select(Game.season, sa_func.count()).group_by(Game.season).order_by(Game.season)
            )
        ).all()
        ...
        with_odds_id = (
            await db.execute(
                select(sa_func.count()).select_from(Game).where(Game.odds_api_id.isnot(None))
            )
        ).scalar()
        total_games = (await db.execute(select(sa_func.count()).select_from(Game))).scalar()
        print()
        ...
        with_scores = (
            await db.execute(
                select(sa_func.count()).select_from(Game).where(Game.home_score_fg.isnot(None))
            )
        ).scalar()
        with_1h = (
            await db.execute(
                select(sa_func.count()).select_from(Game).where(Game.home_score_1h.isnot(None))
            )
        ).scalar()
        ...
        print()
        print("PREDICTIONS:")
        pred_count = (await db.execute(select(sa_func.count()).select_from(Prediction))).scalar()
        pred_with_opening = (
            await db.execute(
        ...
        odds_count = (await db.execute(select(sa_func.count()).select_from(OddsSnapshot))).scalar()
        unique_games_with_odds = (
            await db.execute(select(sa_func.count(OddsSnapshot.game_id.distinct())))
        ).scalar()
        ...
        mr_rows = (
            (
                await db.execute(
                    select(ModelRegistry).order_by(ModelRegistry.created_at.desc()).limit(5)
                )
            )
            .scalars()
            .all()
        ...
        for p in p_rows:
            ...
            print(
                f"  {away} @ {home} [{status}] pred={p.predicted_home_fg:.0f}-{p.predicted_away_fg:.0f}"
...
def main() -> None:
    ...
    # publish-teams
    p_pub = sub.add_parser("publish-teams", help="Generate predictions and publish to Teams")
    ...
    p_backtest.add_argument(
        "--model-version", default=None, help="Filter to a specific model version"
    )
